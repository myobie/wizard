defmodule Wizard.PreviewGenerator.Uploader do
  ...
  def upload_png(remote_path, [from: local_path]) do
    token = upload_token()
    ...
    case request(uri, local_path) do
      {:ok, %HTTPoison.Response{status_code: 201}} -> :ok
      error -> error
    end
  end

  defp request(uri, local_path) do
    HTTPoison.put(to_string(uri), {:file, local_path}, @png_blob_headers)
  end
